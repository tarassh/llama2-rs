...
pub struct TfheTokenizer {
    ...
    /// The client key for the tokenizer
...
impl TfheTokenizer {
    ...
    pub fn new_from_file<P: AsRef<Path>>(
        ...
        Self::build_tokenizer(path, vocab_size, client_key)
    }

    pub fn encode(&mut self, text: &str, bos: i8, eos: i8) -> Vec<FheInt32> {
        ...
        tokens
            .iter()
            .map(|token| FheInt32::try_encrypt(*token, &self.client_key).unwrap())
            .collect()
    }
    ...
    pub fn decode(&self, prev_token: FheInt32, token: FheInt32) -> String {
        let prev_token: i32 = prev_token.decrypt(&self.client_key);
        let token: i32 = token.decrypt(&self.client_key);
